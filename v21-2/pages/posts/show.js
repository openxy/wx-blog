...
Page({
  ...
  touchEnd: function (e) {        
    ...
    if (touchEndX - touchBeginX <= -40 && time < 1000 ) {      
      console.log("下一页", store.getNextId('post', current_id));      
      wx.navigateTo({
        url: './show?id=' + store.getNextId('post',current_id)
      })
    }
    
    // 向左滑动   
    if (touchEndX - touchBeginX >= 40 && time < 1000 ) {            
      console.log("上一页", store.getNextId('post', current_id, true));
      wx.navigateTo({
        url: './show?id=' + store.getNextId('post', current_id,true)
      })
    }
